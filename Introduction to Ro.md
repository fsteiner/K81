...
### M2
M1 is superior to R0 in taking into account that the epidemic is limited by the size of the population, but it assumes that every patient is contagious forever. If we make the assumption that every patient is contagious for a fixed period C, and if we define Ro by **the number of individuals *directly* contaminated by each carrier of the virus *among a population with no immunity***, we have directly:
> Ro = r x C
